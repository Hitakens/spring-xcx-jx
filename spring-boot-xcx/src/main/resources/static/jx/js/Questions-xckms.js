...
function getKmsdata() {
	 ...
	 $.ajax({
         type: "get",
         dataType: "html",
         url: pathName+"/xc/getKmsdata/"+cs, //后台文件的url 就是form的action,用ajax提交表单就不需要给form写action了
...
function showQuestion(id) {
	...
	if(question.questionImg!='无'){
		$(".question")
		.append(
				'<li class="quest-sum"><img class="question-img" alt="" src="'+pathName+'/static/jx/img/xckms/'+question.questionImg+'"></li>')
	}
	for (var i = 0; i < items.length; i++) {
		item = "<li class='question_info' onclick='clickTrim(this)' id='item"
				+ i + "'><input type='checkbox' name='item' value='" + itemList[i]
